...
public class GenUtils {
	...
	public static void generatorCode(Map<String, String> table,
		...
		//表名转换成Java类名
		String className = tableToJava(tableEntity.getTableName(), initData.get(TABLEPREFIX).toString());
		...
		map.put("package", initData.get(PACKAGE));
		map.put("author", initData.get(AUTHOR));
		map.put("email", initData.get(EMAIL));
		...
		for(String template : templates){
			...
			try {
				//添加到zip
				zip.putNextEntry(new ZipEntry(getFileName(template, tableEntity.getClassName(), initData.get(PACKAGE).toString())));
	...
	public static void setConfig(String key, Object value){
		...
		try{
			...
			throw new RRException("写入配置文件失败", e);
		} finally {
			IOUtil.close(fos);
		}
	}
